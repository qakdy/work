...
describe('registration test 8 ru;Проверить создание портфолио при выборе на онбординг скрин   радиокнопку "Мастер-пароль и отпечаток пальца/Face ID(рекомендуется)"  или   "Только мастер пароль (наиболее безопасный)"с пустым полем  "Мое портфолио" и заполенными полями пароль валидными данными', function(){
