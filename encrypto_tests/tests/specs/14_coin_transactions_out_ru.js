...
describe('coin transaction out 14 ru; Check the creation of a transaction when choosing out from the first drop-down menu, leave the default value in the second one, with an empty "quantity" field with the "USD" field filled in by default and choosing a wallet from the drop-down list and pressing the [Сохранить] button', function(){
    ...
    it('STEP 2. From the first drop-down menu select "Исходящее"', function(){  
       ...
       const text_sell = 'Продажа'
       const attr_text_sell = AddTransaction.sell_ru.getAttribute('content-desc')
       expect(text_sell).toBe(attr_text_sell)
    ...
    it('STEP 4. Select Wallet from the drop down list', function(){ 
    ...
    it('STEP 7. Press the [Отмена] button', function(){ 

        AddTransaction.save_ru.click()

    })  

    it('EXPECTED RESULT. Under the fileds "Quantity" an informational message is displayed in red "Пожалуйста, введите количество"', function(){ 


        const usd_red_message = 'Пожалуйста, введите количество'
        const attr_usd_red_message = AddTransaction.red_message_ru.getAttribute('content-desc')
        expect(usd_red_message).toBe(attr_usd_red_message)
