...
import Gmail from '../screenobjects/gmail'



describe('backup/restore test 6 en; Check sending zip file using the selected application', function(){
    ...
    it(' STEP 2. Click on the button on the right on the tile of the zip file', function(){ 
        ...
        Backup_Restore.show_menu.click()

        
    }); 

    it(' STEP 3. Select "Share" from the drop down menu', function(){ 

        Backup_Restore.share.click()

    }); 

    it(' STEP 4. Select app(gmail) to send', function(){ 

        Backup_Restore.gmail.click()

    }); 

    it(' STEP 5. We confirm the actions necessary for sending, if necessary', function(){ 

        Gmail.email_user.setValue('iphonedimon1908@gmail.com')

        driver.pressKeyCode(66)

        const attr_archive =Gmail.archive.getAttribute('displayed')
        expect(attr_archive).toBe('true')

        Gmail.send_btn.click()
