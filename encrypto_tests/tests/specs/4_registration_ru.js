...
describe('registration test 4 ru; Проверить создание портфолио при выборе на онбординг скрин радиокнопку "Мастер-пароль и отпечаток пальца/Face ID(рекомендуется)"  или   "Только мастер пароль (наиболее безопасный)"с разными  паролями в полях "Основной пароль" " Подтвердить пароль" ', function(){
    ...
    it('STEP 3. В поле "Подтвердить пароль" ввести пароль отличный от введенного в поле "Основной пароль"', function(){
    ...
    it('EXPECTED RESULT. Под полем "Подтвердите  пароль"  отображается информационное сообщение  красным цветом  "Пароль не подходит"', function(){
