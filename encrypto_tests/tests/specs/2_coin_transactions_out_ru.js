...
describe('coin transaction out 2 ru; Check the creation of an out-transfer transaction with a changed coin value and a wallet selection from the drop-down list', function(){
    ...
    it('STEP 2. From the first drop-down menu select "Исходящее"', function(){  
       ...
       const text_in = 'Исходящее'
       const attr_text_in = AddTransaction.out_ru.getAttribute('content-desc')
       expect(text_in).toBe(attr_text_in)

       AddTransaction.out_ru.click()

    
    });


    it('STEP 3. Select "Перечисление" from the second drop-down list', function(){ 
       ...
       const text_transfer = 'Перечисление'
       const attr_text_transfer = AddTransaction.transfer_ru.getAttribute('content-desc')
       expect(text_transfer).toBe(attr_text_transfer)

       AddTransaction.transfer_ru.click()
    })
    ...
    it('STEP 5. Enter the value of the coin in the "USD" field manually', function(){ 
    ...
    it('STEP 8. Select Wallet from the drop down list', function(){ 
    ...
    it('STEP 9. Press the [Сохранить] button', function(){ 

        AddTransaction.save_ru.click()

    })  

    it('EXPECTED RESULT. The header displays: - screen name, coin name. - button [<]. The static upper part displays: - the number of purchased coins, the abbreviated name of the coin. The main part of the screen displays: - button [+] at the bottom right.', function(){ 
        ...
        const five_btc_text = '-5.0 BTC'
        const attr_five_btc =Transaction.minus_five_btc.getAttribute('content-desc')
        expect(five_btc_text).toBe(attr_five_btc)
