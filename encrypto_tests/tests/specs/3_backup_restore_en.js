...
describe('backup/restore test 3 en; Check the ability to display the application to send a zip file', function(){
    ...
    it('EXPECTED RESULT. The screen displays: -sent file ;-list of applications with which you can send a file', function(){ 
        ...
        const attr_bluetooth = Backup_Restore.bluetooth.getAttribute('text')
        expect(text_bluetooth).toBe(attr_bluetooth)

        const text_gmail = 'Gmail'
        const attr_gmail = Backup_Restore.gmail.getAttribute('text')
        expect(text_gmail).toBe(attr_gmail)

        const drive_text = 'Drive'
        const attr_drive = Backup_Restore.drive.getAttribute('text')
