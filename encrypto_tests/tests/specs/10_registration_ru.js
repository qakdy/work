...
describe('registration test 10 ru; Проверить создание портфолио при выборе на онбординг скрин   радиокнопку "Мастер-пароль и отпечаток пальца/Face ID(рекомендуется)"  или   "Только мастер пароль (наиболее безопасный)"с пустым полями  "Мое портфолио" и "Подтвердить пароль"', function(){
    ...
    it('STEP 3. Поле  "Подтвердить пароль"  оставить пустым ', function(){
    ...
    it('EXPECTED RESULT. После нажатия кнопки [Создать] Под полем "Мое портфолио"отображается информационное сообщение красным цветом" Пожалуйста, введите имя". Под полем "Подтвердить пароль" отображается информационное сообщение красным цветом" Пожалуйста, введите  подтверждение пароля".  ', function(){
