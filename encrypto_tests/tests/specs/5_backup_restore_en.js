...
describe('backup/restore test 5 en; Test the dropdown menu on the zip file tile', function(){
    ...
    it('EXPECTED RESULT. After clicking on the drop-down menu button on the tile of the zip file, a drop-down menu is displayed with a choice:- "Share"- "Delete" ', function(){ 

        const attr_share = Backup_Restore.share.getAttribute('displayed')
        expect(attr_share).toBe('true')
        
        const attr_delete = Backup_Restore.delete.getAttribute('displayed')
        expect(attr_delete).toBe('true')
