...
describe('privacy policy 1 en; Check the opening of the "Privacy Policy" screen', function(){
    ...
    it('STEP 2. Click the "Privacy Policy" section in the sidebar', function(){  

        const text_privacy_policy = 'Privacy Policy'
        const attr_privacy_policy= Burger_Menu.privacy_policy_en.getAttribute('content-desc')
        expect(text_privacy_policy).toBe(attr_privacy_policy)

        Burger_Menu.privacy_policy_en.click()
        
    });  

    it('EXPECTED RESULT. The "Privacy Policy" window opensin the Header of the page is displayed: - screen title "Privacy Policy"; - button [<].', function(){  
        
        const text_privacy_policy = 'Privacy Policy'
        const attr_privacy_policy= Privacy_Policy.header_en.getAttribute('content-desc')
        expect(text_privacy_policy).toBe(attr_privacy_policy)

        const attr_back_btn = Privacy_Policy.back_btn.getAttribute('displayed')
        expect(attr_back_btn).toBe('true')

        Privacy_Policy.introdaction_text.waitForDisplayed({timeout:10000})

        const introdaction_text = 'Introduction'
        const attr_intrduction = Privacy_Policy.introdaction_text.getAttribute('text')
        expect(introdaction_text).toBe(attr_intrduction)
